...
description: Operate E2B agent sandboxes using CLI. Use when user needs to run code in isolation, test packages, execute commands safely, or work with binary files in a sandbox environment. Keywords: sandbox, e2b, isolated environment, run code, test code, safe execution, sisyphus, subagent, opencode, glm-4.7.
compatibility:
  - opencode: "1.0+"
  - claude-code: "1.0+"
---
...
|----------|------|-----|------|----------|
| `claude-node22-python-sqlite` | 2 | 512MB | ~$0.05/hr | Simple apps (default) |

**What's included**: Node.js 22, npm, uv (Python package manager), sqlite3 (database CLI)
**What projects install**: Vue, Vite, Pinia, FastAPI, etc. (via npm/uv for version flexibility)
**Note**: 512MB is for template build resources. Runtime sandboxes auto-scale based on workload.

Build custom templates: See `build_accurate_template.py` for reference.
